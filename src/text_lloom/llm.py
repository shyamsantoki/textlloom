...
if "OR_API_KEY" not in os.environ:
    raise Exception(
        "API key not found. Please set the OPENAI_API_KEY environment variable by running: `os.environ['OR_API_KEY'] = 'your_key'`"
    )

client = AsyncOpenAI(
    api_key=os.environ.get("OR_API_KEY"),
    base_url=os.environ.get("OR_API_BASE_URL"),
)
embed_client = OpenAI(
...
RATE_LIMITS = {
    ...
    "meta-llama/llama-3-70b-instruct": (10, 10),  # = 10*6 = 60 rpm
}
...
COSTS = {
    ...
    "meta-llama/llama-3-70b-instruct": [0.03 / 1000, 0.06 / 1000],
}
...
async def multi_query_gpt_wrapper(
    ...
    if not batched:
        # Non-batched version
        tasks = [
            multi_query_gpt(model_name, prompt_template, args, temperature=temperature)
...
def get_embeddings(embed_model_name, text_vals):
    ...
    chunked_text_vals = np.array_split(
        text_vals_mod, np.arange(chunk_size, num_texts, chunk_size)
    )
    embeddings = []
