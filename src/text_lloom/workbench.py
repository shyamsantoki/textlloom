...
if __package__ is None or __package__ == "":
    ...
    from llm import EMBED_COSTS, RATE_LIMITS, get_token_estimate
else:
...
class lloom:
    def __init__(
        ...
        self.distill_model_name = (
            distill_model_name  # Distill operators (filter and summarize)
        )
        ...
        if "OPENAI_API_KEY" not in os.environ:
            raise Exception(
                "API key not found. Please set the OPENAI_API_KEY environment variable by running: `os.environ['OPENAI_API_KEY'] = 'your_key'`"
            )
    ...
    def estimate_gen_cost(self, params=None, verbose=False):
        if params is None:
            ...
            print(
                f"No parameters provided, so using auto-suggested parameters: {params}"
            )
        ...
        filter_in_tokens = np.sum(
            [
                get_token_estimate(filter_prompt + doc, self.distill_model_name)
                for doc in self.in_df[self.doc_col].tolist()
            ]
        )
        quotes_tokens_per_doc = params["filter_n_quotes"] * est_quote_tokens
        filter_out_tokens = quotes_tokens_per_doc * len(self.in_df)
        est_cost["distill_filter"] = calc_cost_by_tokens(
            self.distill_model_name, filter_in_tokens, filter_out_tokens
        )

        # Summarize: create n_bullets for each doc
        summ_prompt_tokens = get_token_estimate(
            summarize_prompt, self.distill_model_name
        )
        summ_in_tokens = np.sum(
            [
                (summ_prompt_tokens + quotes_tokens_per_doc)
                for _ in range(len(self.in_df))
            ]
        )
        bullets_tokens_per_doc = params["summ_n_bullets"] * est_bullet_tokens
        summ_out_tokens = bullets_tokens_per_doc * len(self.in_df)
        est_cost["distill_summarize"] = calc_cost_by_tokens(
            self.distill_model_name, summ_in_tokens, summ_out_tokens
        )
        ...
        n_bullets_per_cluster = (
            params["summ_n_bullets"] * len(self.in_df)
        ) / est_n_clusters
        synth_prompt_tokens = get_token_estimate(
            synthesize_prompt, self.synth_model_name
        )
        synth_in_tokens = np.sum(
            [
                (synth_prompt_tokens + (est_bullet_tokens * n_bullets_per_cluster))
                for _ in range(est_n_clusters)
            ]
        )
        synth_out_tokens = (
            params["synth_n_concepts"] * est_n_clusters * est_concept_tokens
        )
        est_cost["synthesize"] = calc_cost_by_tokens(
            self.synth_model_name, synth_in_tokens, synth_out_tokens
        )
        ...
        est_cost["review"] = calc_cost_by_tokens(
            self.synth_model_name, rev_in_tokens, rev_out_tokens
        )
    ...
    def estimate_score_cost(
        self, n_concepts=None, batch_size=5, get_highlights=True, verbose=False
    ):
        ...
        all_doc_tokens = np.sum(
            [
                get_token_estimate(doc, self.score_model_name)
                for doc in self.df_to_score[self.doc_col].tolist()
            ]
        )  # Tokens to encode all documents
        score_in_tokens = all_doc_tokens + (
            n_batches * (score_prompt_tokens + est_concept_tokens)
        )
        score_out_tokens = est_score_json_tokens * n_concepts * len(self.in_df)
        est_cost = calc_cost_by_tokens(
            self.score_model_name, score_in_tokens, score_out_tokens
        )
    ...
    def auto_suggest_parameters(
        self, sample_size=None, target_n_concepts=20, debug=False
    ):
    ...
    def summary(self, verbose=True):
        ...
        print(
            f"{self.bold_txt('Total time')}: {total_time:0.2f} sec ({(total_time/60):0.2f} min)"
        )
        if verbose:
    ...
    def show_prompt(self, step_name):
        ...
        if step_name in steps_to_prompts:
            ...
            raise Exception(
                f"Operator `{step_name}` not found. The available operators for custom prompts are: {list(steps_to_prompts.keys())}"
            )
    ...
    def validate_prompt(self, step_name, prompt):
        ...
        for req in reqs:
            ...
            if template_str not in prompt:
                raise Exception(
                    f"Custom prompt for `{step_name}` is missing required template field: `{req}`. All required fields: {reqs}. For example, this is the default prompt template:\n{self.show_prompt(step_name)}"
                )
    ...
    async def gen(
        ...
        if (filter_n_quotes > 1) and (custom_prompts["distill_filter"] is not None):
            ...
            with self.spinner_wrapper() as spinner:
                ...
                self.df_to_score = (
                    df_filtered  # Change to use filtered df for concept scoring
                )
        ...
        for i in range(n_synth):
            ...
            df_concepts["synth_doc_col"] = (
                df_concepts[f"{concept_col_prefix}_name"]
                + ": "
                + df_concepts[f"{concept_col_prefix}_prompt"]
            )
    ...
    async def select_auto(self, max_concepts):
        ...
        selected_concepts = await review_select(
            self.concepts, max_concepts, self.synth_model_name, self.rate_limits
        )
    ...
    async def score(
        self, c_ids=None, batch_size=1, get_highlights=True, ignore_existing=True
    ):
        concepts = {}
        active_concepts = self.__get_active_concepts()
        if c_ids is None:
        ...
        if ignore_existing:
            concepts = {
                c_id: c for c_id, c in concepts.items() if c_id not in self.results
            }

        # Run cost estimation
        self.estimate_score_cost(
    ...
    def __get_concept_from_name(self, name):
        if name == "Outlier":
            return Concept(
                name="Outlier", prompt=OUTLIER_CRITERIA, example_ids=[], active=True
            )
        for c_id, c in self.concepts.items():
            if c.name == name:
                return c
            ...
            if df[col].dtype == object:
                df[col] = df[col].apply(
                    lambda x: (
                        ...
                        else x.encode("unicode-escape").decode("ascii")
                    )
                )
        return df
    ...
    def get_score_df(self):
        ...
        active_score_dfs = [
            self.results[c_id]
            for c_id in active_concepts.keys()
    ...
    def __get_concept_highlights(
        self, c, threshold=1.0, highlight_col="highlight", lim=3
    ):
        if c.name == "Outlier":
            return []
    ...
    def __get_df_for_export(self, item_df, threshold=1.0, include_outliers=False):
        ...
        df = (
            matched.groupby(by=["id", "concept"])
            .count()
            .reset_index()[["concept", self.doc_col]]
        )
        concepts = [
            self.__get_concept_from_name(c_name) for c_name in df.concept.tolist()
        ]
        ...
        df["highlights"] = [
            self.__get_concept_highlights(c, threshold) for c in concepts
        ]
    ...
    def export_json(self, threshold=1.0):
        ...
        active_concepts = self.__get_active_concepts()
        concepts_dict = [
            format_dict(c.to_dict()) for c_id, c in active_concepts.items()
        ]
    ...
    def submit(self):
        ...
        # Gather user inputs
        print(
            "Thank you for using LLooM and submitting your work! We would love to hear more about your analysis."
        )
        print(
